...
# # def is_side_face(image_path_or_crop, asymmetry_threshold=0.35):
# #     """
# #     True  -> Side/Tilted face (eyes/nose/lips missing ya asymmetric)
# #     False -> Valid Frontal Face
# #     """
# #     img = cv2.imread(image_path_or_crop) if isinstance(image_path_or_crop, str) else image_path_or_crop
# #     if img is None:
# #         return True
...
# #     # Step 1: Face Mesh khud fail ho gaya -> eyes/nose/lips detect hi nahi hue
# #     if not results.multi_face_landmarks:
# #         print("🚫 No landmarks found — clear side/profile face")
# #         return True

# #     landmarks = results.multi_face_landmarks[0].landmark

# #     # Step 2: Required points image ke andar valid range me hain ya nahi
# #     required_points = LEFT_EYE + RIGHT_EYE + [NOSE_TIP] + MOUTH
# #     for idx in required_points:
# #         lm = landmarks[idx]
# #         if not (0.0 <= lm.x <= 1.0 and 0.0 <= lm.y <= 1.0):
# #             print(f"🚫 Landmark {idx} out of frame — feature missing")
# #             return True

# #     # Step 3: Geometric asymmetry (eye-nose-eye triangle) — moderate tilt catch karega
# #     left_eye_pt = np.array([landmarks[33].x * w, landmarks[33].y * h])
# #     right_eye_pt = np.array([landmarks[263].x * w, landmarks[263].y * h])
# #     nose_pt = np.array([landmarks[1].x * w, landmarks[1].y * h])

# #     dist_left = np.linalg.norm(nose_pt - left_eye_pt)
# #     dist_right = np.linalg.norm(nose_pt - right_eye_pt)
...
# #         print(f"🚫 Side Face Rejected: asymmetry {asymmetry_ratio:.2f} > {asymmetry_threshold}")
# #         return True

# #     return False  # ✅ Valid frontal face



# def is_side_face(face_obj, yaw_threshold=22.0, asymmetry_threshold=0.20):
#     """
#     True  -> Side/Tilted face
#     False -> Valid Frontal Face
#     Fully defensive: kabhi bhi raise nahi karega, worst case True return karega.
#     """
#     try:
#         # 1️⃣ Pose-based check (agar available hai)
#         pose = getattr(face_obj, 'pose', None)
#         if pose is not None:
#             try:
#                 pose_arr = np.asarray(pose, dtype=float)
#                 if pose_arr.ndim == 1 and pose_arr.shape[0] >= 2:
#                     yaw = abs(float(pose_arr[1]))
#                     if yaw > yaw_threshold:
#                         return True
#             except (TypeError, ValueError):
#                 # pose corrupt/None-containing — ignore, fallback to kps check
#                 pass

#         # 2️⃣ Keypoint-based check
...
#         if np.isnan(kps_arr).any():
#             return True

#         left_eye, right_eye, nose = kps_arr[0], kps_arr[1], kps_arr[2]
...
#         asymmetry_ratio = abs(dist_left - dist_right) / max_dist
#         return asymmetry_ratio > asymmetry_threshold

#     except Exception as e:
#         print(f"⚠️ is_side_face internal check failed: {e}")
#         return True
...
#             for candidate, result in zip(batch_candidates, human_results):
#                 if result is None:
...
# _RIGHT_EYE_IDX = 263
# _NOSE_TIP_IDX = 1
# _MOUTH_IDX = [61, 291, 13, 14]
# _REQUIRED_LANDMARKS = [_LEFT_EYE_IDX, _RIGHT_EYE_IDX, _NOSE_TIP_IDX] + _MOUTH_IDX
...
_NOSE_TIP_IDX = 1
_CHIN_IDX = 152
_LEFT_EYE_CORNER_IDX = 33
_RIGHT_EYE_CORNER_IDX = 263
...
def is_side_face(face_crop, yaw_threshold=25.0, pitch_threshold=20.0):
    """
    face_crop: RGB numpy array (cropped face image).
    True  -> Side face (left-right turned) YA tilted (upar/neeche jhuka, jaise khaana khate waqt)
